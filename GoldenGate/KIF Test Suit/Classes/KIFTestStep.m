...
+ (id) stepToInterfaceOrientation: (UIInterfaceOrientation) toInterfaceOrientation {
...
+ (id)stepToDismissPopover;
{
    return [self stepWithDescription:@"Dismiss the popover" executionBlock:^(KIFTestStep *step, NSError **error) {
        ...
        UIView *dimmingView = [[[windows objectAtIndex:0] subviewsWithClassNamePrefix:@"UILayoutContainerView"] lastObject];
...
+ (id) stepToDismissAlertWithLabel: (NSString *) label
{
    return [KIFTestStep stepWithDescription:[NSString stringWithFormat:@"Find the alert dialog with label '%@' and dismiss it", label] executionBlock:^KIFTestStepResult(KIFTestStep *step, NSError **error) {
        return [self dismissAlertWithLabel:label andButtonText:@"OK" andError:error];
    }];
}
+(KIFTestStepResult) dismissAlertWithLabel:(NSString*)label andButtonText:(NSString*)buttonText andError:(NSError**)error;
{
    UIAlertView* alert = [self getAlertView];
    KIFTestCondition(alert, error, @"Alert window was not found among available windows.");
    ...
    for (UIView *view in [alert subviews]) {
        if ([[[view class] description] isEqualToString:@"UIAlertButton"]) {
            ...
            if ([[btn accessibilityLabel] isEqual:buttonText]) {
                [view tap];
                result = true;
                break;
            }
    ...
    KIFTestCondition(result, error, @"Alert with title %@ not found. Expected: %@ but got: %@", label, label, alertTitle);
...

{
    ...
    NSLog(@"Current application has %d windows.", [windows count]);
...
//Rajeev Start
+ (id)stepToSwipeViewWithAccessibilityLabelOne:(NSString *)label inDirection:(KIFSwipeDirection)direction
{
    // The original version of this came from http://groups.google.com/group/kif-framework/browse_thread/thread/df3f47eff9f5ac8c
    NSString *directionDescription = nil;
    ...
    return [KIFTestStep stepWithDescription:description executionBlock:^(KIFTestStep *step, NSError **error) {
        ...
        KIFDisplacement swipeDisplacement = [self _displacementForSwipingInDirectionOne:direction];
...
//Rajeev Start
#define MAJOR_SWIPE_DISPLACEMENT1 600
#define MINOR_SWIPE_DISPLACEMENT1 6
//Rajeev End

#define MAJOR_SWIPE_DISPLACEMENT 150
#define MINOR_SWIPE_DISPLACEMENT 3
...
//Rajeev Start
+ (KIFDisplacement)_displacementForSwipingInDirectionOne:(KIFSwipeDirection)direction
{
    switch (direction)
    {
            // As discovered on the Frank mailing lists, it won't register as a
            ...
            // slight orthogonal offset too.
        case KIFSwipeDirectionRight:
            return CGPointMake(MAJOR_SWIPE_DISPLACEMENT1, MINOR_SWIPE_DISPLACEMENT1);
            break;
        case KIFSwipeDirectionLeft:
            return CGPointMake(-MAJOR_SWIPE_DISPLACEMENT1, MINOR_SWIPE_DISPLACEMENT1);
            break;
        case KIFSwipeDirectionUp:
            return CGPointMake(MINOR_SWIPE_DISPLACEMENT1, -MAJOR_SWIPE_DISPLACEMENT1);
            break;
        case KIFSwipeDirectionDown:
            return CGPointMake(MINOR_SWIPE_DISPLACEMENT1, MAJOR_SWIPE_DISPLACEMENT1);
            break;
