//
//  EntityVideoPlayBackViewController.m
//  GoldenGate
//
...
@interface EntityVideoPlayBackViewController ()

// UI elements
@property (weak, nonatomic) IBOutlet UILabel *videoValueLable;
...
@property(readwrite)int Video_slide;
...
@implementation EntityVideoPlayBackViewController
enum
{
...
{
    EntityVideoPlayBackViewController *videoPlaybackViewController = [[EntityVideoPlayBackViewController alloc] initWithVideo:video entity:dispEntity];
    ...
    if (dispEntity == nil)
    {
        [[GGBackgroundOperationQueue sharedInstance] addOperationWithBlock:^
         {
             NSError *error = nil;
             Show *showForVideo = [[VimondStore showStore] showWithId:video.channelID error:&error];
             [[NSOperationQueue mainQueue] addOperationWithBlock:^
              {
                  videoPlaybackViewController.currentEntity = showForVideo;
              }];
         }];
    }
}
...
- (id)initWithVideo:(Video *)video entity:(DisplayEntity *)entity
{
...
- (void)viewDidLayoutSubviews
{
    ...
    self.playlistTray.trayTabView.accessibilityLabel=@"watchnext";
    self.myChannelsTray.trayTabView.accessibilityLabel=@"subscrip";
}
...
- (void)viewDidLoad
{
    ...
    [self updateFromEntity:self.currentEntity];
    [self updateFromVideo:self.currentVideo];
     self.volumeView.volumeValueLabel.text = [NSString stringWithFormat:@"%.1f", self.volumeView.volumeSlider.value];
    UIButton *videoSliderButton = [UIButton buttonWithType:UIButtonTypeCustom];
    videoSliderButton.frame =CGRectMake(0.0, 0.0, 1.0, 1.0);
    videoSliderButton.accessibilityLabel = @"videoSlider";
    videoSliderButton.isAccessibilityElement = YES;
    [self.videoProgressSlider addSubview:videoSliderButton];
    
    [videoSliderButton addTarget:self action:@selector(videoSliderButtonAction) forControlEvents:UIControlEventTouchUpInside];
    
    self.playPauseButton.accessibilityLabel = @"Play&Pause";
}

-(void)videoSliderButtonAction
{
    //Rajeev Start
    
    //    if (self.videoProgressSlider.value == 0.0) {
    //        [self.videoProgressSlider setValue:100.0 animated:YES];
    //        [self progressSliderValueChanged];
    //    }
    //    else {
    //        [self.videoProgressSlider setValue:0.0 animated:YES];
    //        [self progressSliderValueChanged];
    //    }
    
    if (self.Video_slide == 0) {
        self.Video_slide = 50;
        [self.videoProgressSlider setValue:0.0 animated:YES];
        //[self performSelector:@selector(progressSliderValueChanged) withObject:nil afterDelay:5.0];
        
        [self progressSliderValueChanged];
    }else if (self.Video_slide == 50) {
        self.Video_slide = 40;
        [self.videoProgressSlider setValue:40.0 animated:YES];
        //[self performSelector:@selector(progressSliderValueChanged) withObject:nil afterDelay:5.0];
        [self progressSliderValueChanged];
    }else if (self.Video_slide == -10) {
        [self.videoProgressSlider setValue:50.0 animated:YES];
        //[self performSelector:@selector(progressSliderValueChanged) withObject:nil afterDelay:5.0];
        [self progressSliderValueChanged];
    }else if (self.Video_slide == 40) {
        self.Video_slide = -10;
        [self.videoProgressSlider setValue:-10.0 animated:YES];
        //[self performSelector:@selector(progressSliderValueChanged) withObject:nil afterDelay:5.0];
        [self progressSliderValueChanged];
    }
...
- (void)updateFromEntity:(DisplayEntity *)dispEntity
{
    if (self.view == nil || !dispEntity)
    {
        return;
    }
    ...
    self.updateFromChannelOperation = [self runOnUIThread:^
                                       {
                                           ...
                                           [self runOnBackgroundThread:^
                                            {
                                                NSError *error = nil;
                                                SearchResult *searchResult = [[VimondStore videoStore] videosForEntity:dispEntity pageIndex:0 pageSize:kMaxPlaylistLength sortBy:ProgramSortByPublishedDateDesc
                                                                                                                  error:&error];
...
- (void)updateChannelHighlightInTrayView
{
    ...
    //if the entity is of type show it is not high lighted
    if ([self.currentEntity isKindOfClass:[Channel class]]) {
        [self.myChannelsTray selectDynamicContentAtIndex:[self indexOfChannel:(Channel*)self.currentEntity]];
    }
                                         ...
                                         [self runOnBackgroundThread:^
                                          {
                                           ...
                                           //   Video *videoToPlay = hasAccess ? video : video.previewVideo;
                                              Playback *playback = [playbackStore playbackForVideo:video error:&error];
                                              ...
                                              PlayProgress *newProgress = [playbackStore playProgressForVideo:video error:nil];
                                              ...
                                              [[GGUsageTracker sharedInstance] trackPlayVideo:video preview:!hasAccess];
...
- (void)updateVideoDurationGUI
{
    ...
    if ((CMTIMERANGE_IS_VALID(seekableTimeRange))&&(!CMTIMERANGE_IS_EMPTY(seekableTimeRange)))
    {
        ...
        if (!self.isUserDraggingSeeker && !self.isPlayerSeeking)
        {
            ...
            self.videoValueLable.text = [NSString stringWithFormat:@"%.1f\n%d", (([self.playerLayerView.ptPlayer getBitrate])/1048576.0),(int)self.videoProgressSlider.value];
        }
    }
    else
    {
...
- (void)setCurrentEntity:(DisplayEntity *)currentEntity
{
    _currentEntity = currentEntity;
    [self updateFromEntity:currentEntity];
}
...
- (void)seekToTime:(NSTimeInterval)time
{
    ...
    if (CMTIMERANGE_IS_VALID([self currentlySeekableTimeRange]))
    {
        ...
        EntityVideoPlayBackViewController* selfController = self;
...
{
    int indexForCurrentItem = -1;
    
    if ([self.currentEntity isKindOfClass:[Channel class]]) {
        indexForCurrentItem = self.playlistTray == trayView ? [self indexOfVideo:self.currentVideo] : [self indexOfChannel:(Channel*)self.currentEntity];
    }else{
        if (self.playlistTray == trayView) {
            indexForCurrentItem = [self indexOfVideo:self.currentVideo];
        }
    }
...
- (void)setPlayBackControlsVisible:(BOOL)visible
{
    ...
    UIButton *volumeSliderButton = [UIButton buttonWithType:UIButtonTypeCustom];
    volumeSliderButton.frame =CGRectMake(0.0, 0.0, 1.0, 1.0);
    volumeSliderButton.accessibilityLabel = @"volumeSlider";
    volumeSliderButton.isAccessibilityElement = YES;
    [self.volumeView addSubview:volumeSliderButton];
    
    [volumeSliderButton addTarget:self.volumeView action:@selector(volumeSliderButtonAction) forControlEvents:UIControlEventTouchUpInside];
...
- (void)didTapChannelCell:(ChannelCellView *)sender
{
    if (sender.channel)
    {
        self.currentEntity = sender.channel;
...
{
    NSString* strToView = [NSString stringWithFormat:@"/Video_Player-%d-%@",self.currentVideo.identifier,self.currentVideo.title];
    return strToView;
}
